...
{  
    ...
    {  
        ...
        _RimLightIntensity("RimLightIntensity",Range(0,1)) = 0.5  
    ...
    {  
        ...
        {   
            ...
            {  
			    ...
			    float3 lightDir_rim = normalize(mul((float3x3)UNITY_MATRIX_V,_RimLightDir));  
                half NdotL_rim = (dot(normalDir, lightDir_rim)+1.0)*0.5;  
                half rimlight_term =NdotL_rim ;  
                half toon_rim =saturate((rimlight_term- _ToonTheshold)*20);  
                half3 rimlight_color = (_RimLightColor.rgb+base_color)*0.5;  
                half3 final_rimlight = toon_rim * rimlight_color*_RimLightIntensity;  
  
  
                //輸出  
                half3 Final_color = (Final_diffuse + Final_spec+final_rimlight)*inner_line_color;  
